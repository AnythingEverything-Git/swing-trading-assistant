...
import { listIntradayPractice, listIntradaySessions } from './intraday/api'
import {
  RISK_STORAGE_KEY,
  liveDeskCapital,
  readStoredRisk,
...
function calculateScanPositionSize(
  ...
  let riskBudget = (equity * risk) / 100
  if (Number.isFinite(absCap) && absCap > 0) {
    riskBudget = Math.min(riskBudget, absCap)
  }
  let quantity = Math.floor(riskBudget / perShare)
...
function App() {
  ...
  const [riskProfile, setRiskProfile] = useState<RiskProfile>(() => readStoredRisk())
  const swingEquity = riskProfile.swingEquity
  const intradayEquity = riskProfile.intradayEquity
  const reserveEquity = riskProfile.reserveEquity
  const riskPercent = riskProfile.riskPercent
  const swingRiskPerTradeInr = riskProfile.swingRiskPerTradeInr
  const swingMaxOpenRiskInr = riskProfile.swingMaxOpenRiskInr
  const intradayRiskPerTradeInr = riskProfile.intradayRiskPerTradeInr
  const intradayMaxOpenRiskInr = riskProfile.intradayMaxOpenRiskInr
  const intradayDailyLossInr = riskProfile.intradayDailyLossInr
  /** Intraday live capital (allocated + closed practice PnL); swing live comes from paper book. */
  const [liveIntradayCapital, setLiveIntradayCapital] = useState(
    () => Number(riskProfile.intradayEquity) || 0,
  )
  ...
  /** Allocated + closed swing PnL — used for Find Setups / research sizing. */
  const sizingSwingEquity = useMemo(
    () => String(liveDeskCapital(Number(swingEquity) || 0, paperBook?.trades ?? [])),
    [swingEquity, paperBook?.trades],
  )

  const sizingIntradayEquity = useMemo(
    () => String(Math.max(0, liveIntradayCapital)),
    [liveIntradayCapital],
  )
  ...
  useEffect(() => {
    localStorage.setItem(RISK_STORAGE_KEY, serializeRiskProfile(riskProfile))
  }, [riskProfile])

  useEffect(() => {
    setScanResult((current) =>
      current
        ? withPositionSizing(current, sizingSwingEquity, riskPercent, swingRiskPerTradeInr)
        : current,
    )
  }, [sizingSwingEquity, riskPercent, swingRiskPerTradeInr])

  function applyRiskProfile(next: RiskProfile) {
    setRiskProfile(next)
  }

  function setIntradayEquityFromDesk(value: string) {
    const intra = Number(value)
    if (!Number.isFinite(intra) || intra < 0) return
    applyRiskProfile(
      rebuildProfile({
        allocationMode: 'manual',
        totalEquity: riskProfile.totalEquity,
        swingPct: riskProfile.swingPct,
        intradayPct: riskProfile.intradayPct,
        reservePct: riskProfile.reservePct,
        swingEquity: riskProfile.swingEquity,
        intradayEquity: String(intra),
        reserveEquity: riskProfile.reserveEquity,
        swingRiskPerTradeInr: riskProfile.swingRiskPerTradeInr,
        swingMaxOpenRiskInr: riskProfile.swingMaxOpenRiskInr,
        intradayRiskPerTradeInr: riskProfile.intradayRiskPerTradeInr,
        intradayMaxOpenRiskInr: riskProfile.intradayMaxOpenRiskInr,
        intradayDailyLossInr: riskProfile.intradayDailyLossInr,
      }),
    )
  }

  const handlePracticeLiveCapital = useCallback((next: { swing: number; intraday: number }) => {
    setLiveIntradayCapital(next.intraday)
  }, [])

  const refreshLiveIntradayCapital = useCallback(async () => {
    const allocated = Number(intradayEquity) || 0
    try {
      const { sessions } = await listIntradaySessions(baseUrl, 12)
      const all: Array<{ status: string; realized_pnl?: string | number | null }> = []
      for (const session of sessions) {
        try {
          const listed = await listIntradayPractice(baseUrl, session.id)
          all.push(...listed.trades)
        } catch {
          /* skip session without practice */
        }
      }
      setLiveIntradayCapital(liveDeskCapital(allocated, all))
    } catch {
      setLiveIntradayCapital(allocated)
    }
  }, [baseUrl, intradayEquity])

  useEffect(() => {
    void refreshLiveIntradayCapital()
  }, [refreshLiveIntradayCapital])
  ...
  async function openScanHistoryRun(runId: number) {
    ...
    try {
      ...
      setScanResult(withPositionSizing(payload, sizingSwingEquity, riskPercent, swingRiskPerTradeInr))
  ...
  async function exportScanHistoryCsv(runId: number) {
    ...
    try {
      ...
      const sized = withPositionSizing(payload, sizingSwingEquity, riskPercent, swingRiskPerTradeInr)
  ...
  const inspectDeductionSteps = useMemo(() => {
    ...
    return deductionStepsForOpportunity(selectedOpportunity, sizingSwingEquity, riskPercent)
  }, [selectedOpportunity, sizingSwingEquity, riskPercent])
    ...
    try {
      const response = await fetch(`${baseUrl}/api/v1/backtest/run`, {
        ...
        body: JSON.stringify({
          ...
          account_equity: sizingSwingEquity,
          risk_percent: riskPercent,
          max_risk_amount: swingRiskPerTradeInr,
  ...
  }, [baseUrl, refreshLiveIntradayCapital])
    ...
    try {
      const payload = await runScanAndWait(
        baseUrl,
        {
          ...
          account_equity: sizingSwingEquity || undefined,
          risk_percent: riskPercent || '1',
          max_risk_amount: swingRiskPerTradeInr || undefined,
    ...
    <main className="app-shell">
          ...
          >
            <span className="topbar-capital-line">
              ...
              <strong>
                ...
                }).format(Number(sizingSwingEquity) || 0)}
              </strong>
            </span>
            <span className="topbar-capital-line">
              <span>Intraday</span>
              <strong>
                {new Intl.NumberFormat('en-IN', {
                  ...
                  currency: 'INR',
                  maximumFractionDigits: 0,
                }).format(Number(sizingIntradayEquity) || 0)}
              </strong>
            </span>
            <span className="topbar-capital-line">
              <span>Reserve</span>
      ...
      >
        ...
        <FindSetupsCriteriaBar
          ...
          onRiskChange={(value) => {
            ...
            const abs = String(Math.round((Number(swingEquity) * pct) / 100))
            applyRiskProfile(
              rebuildProfile({
                totalEquity: riskProfile.totalEquity,
                swingPct: riskProfile.swingPct,
                ...
                <div className="field-row two-col">
                  <div className="field-group">
                    ...
                    <input
                      ...
                      step="0.01"
                      value={swingEquity}
                      onChange={() => setCapitalRiskOpen(true)}
                      onClick={() => setCapitalRiskOpen(true)}
                      readOnly
                      title="Edit allocation in Capital & risk"
                    />
                  </div>
                  <div className="field-group">
      ...
      {activeView === 'intraday' && (
        <IntradayDesk
          ...
          onEquityChange={setIntradayEquityFromDesk}
          onPracticeMutated={() => {
            void refreshLiveIntradayCapital()
          }}
          liveSizingEquity={sizingIntradayEquity}
          riskCaps={{
            maxRiskPerTradeInr: intradayRiskPerTradeInr,
            maxOpenRiskInr: intradayMaxOpenRiskInr,
            dailyLossInr: intradayDailyLossInr,
          }}
          reserveEquity={reserveEquity}
      ...
      {activeView === 'research' && (
        <ResearchDesk
          ...
          accountEquity={sizingSwingEquity}
          riskPercent={riskPercent}
          formatPrice={formatPrice}
          formatNumber={formatNumber}
          formatPercent={formatPercent}
      ...
      {activeView === 'practice' && (
        <PracticeBook
          ...
          onLiveCapitalChange={handlePracticeLiveCapital}
          onOpenSwing={() => setActiveView('scan')}
          onOpenIntraday={() => setActiveView('intraday')}
        />
      )}
      ...
      {activeView === 'account' && (
        <AccountShell
          ...
          profile={riskProfile}
          onProfileChange={applyRiskProfile}
          paperTradingEnabled={paperTradingEnabled}
