...
Live (after token):
    MARKET_DATA_SOURCE=upstox UPSTOX_ACCESS_TOKEN=... python scripts/refresh_market_data.py
"""
...
async def refresh(*, universe_name: str, start: datetime | None, end: datetime | None) -> None:
    ...
    try:
        async with sessionmaker() as session:
            if source == "demo":
                ...
                print(f"Demo refresh complete source=demo universe={universe_name}")
            ...
            try:
                ...
                result = await multi.ingest_universe(universe, "1d", resolved_start, resolved_end)
                await session.commit()
            finally:
                await factory.shutdown()
            print(f"Live refresh complete source=upstox universe={universe_name}")
            print(f"  success={result.success_count} failure={result.failure_count}")
...
def main() -> None:
    ...
    _run_async(refresh(universe_name=args.universe, start=args.start, end=args.end))
